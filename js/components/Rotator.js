import { shuffleElements, camelToDash, getFloat, getPosition, debounce, isPercent, getInt, withinRange, isEmptyStr, isNone, getPosInt, capitalize, getNonNegInt, getValue, getTransformProperty, getEasing } from '../util/util.js';
import { FROM_KEYFRAME, TO_KEYFRAME, OPPOSITE_LAYER, CHROME, SUPPORT, ANDROID2, IS_TOUCH, PREFIX, SIDES, OPPOSITE_SIDE } from '../constants.js';
...
Rotator.prototype = {
  ...
  init: function () {
    ...
    if (this._options.startIndex === 'random') {
    ...
    if (IS_TOUCH) {
      if (!isNone(this._options.swipe)) {
        if (this._options.swipeDirection === 'reverse') {
      ...
      this._$outmost.on('mouseenter' + this._namespace, $.proxy(this.pause, this)).on('mouseleave' + this._namespace, $.proxy(this.play, this));
    }
  ...
  initStage: function () {
    ...
    if (!isNone(this._options.timer)) {
      ...
      } else if (this._options.timer === 'pie' && SUPPORT.animation && this._transform) {
  ...
  initItems: function () {
    ...
    const kenBurns = ['kbEffect', 'kbDuration', 'kbEasing'];
    const props = ['columns', 'rows', 'interval', 'direction', 'order', 'alternate', 'autoReverse',
    ...
    this._$items.each($.proxy(function (n, el) {
      const $item = $(el).data('ready', false);
      let $img = $item.children('img').first();
  ...
  injectKeyframes: function ($item) {
    try {
      ...
      const rules = this._sheet.rules || this._sheet.cssRules;
  ...
  initCPanel: function () {
    ...
    if (this._options.cpanelOrientation === 'vertical') {
      ...
      if (this._cpPosition.x === 'center') {
        this._cpPosition.x = 'left';
      }
    } else {
      this._orientation = 'horizontal';
      if (this._cpPosition.y === 'center') {
    ...
    if ($el.length) {
      ...
      const dim = data.dim;
      const outerDim = data.outerDim;

      // position control panel
      if (this._cpPosition[data.coord] === 'center') {
  ...
  setNavOffset: function () {
    ...
    const direction = (this._cpPosition.x === 'left' ? 'prev' : 'next');
    const selector = '>.br-' + direction + '-wrapper';
    ...
    if (this._$outmost.hasClass('br-outer-navs')) {
      ...
      if ($container.find(selector).length) {
        $container.find(selector).find('>.br-nav-thumb').css(prop).data({ margin }).end().width($container.find(selector).children().brTotalWidth(true));
      } else {
  ...
  initButtonGroup: function () {
    ...
    let $last = this._$cpanel.children().last();
  ...
  setInnerCPanel: function () {
    this._$cpWrapper.css(this._cpPosition[this._orientation === 'vertical' ? 'x' : 'y'], 0);
  ...
  setOuterHorizontalCPanel: function () {
    ...
    const size = this._$cpWrapper.height();
  ...
  setOuterVerticalCPanel: function () {
    ...
    const size = this._$cpWrapper.width();
  ...
  initButtons: function () {
    ...
    if (this._options.navButtons === true || this._options.navButtons === 'small') {
      $prevButton = $('<div/>', {
  ...
  initNavThumbs: function () {
    if (!IS_TOUCH && this._options.navThumbs && $.inArray(this._options.navButtons, ['large', 'outside', 'outer']) > -1) {
      let $prevNav; let $nextNav;
      let $prevWrapper = $('<div/>', { class: 'br-prev-wrapper', data: { pos: 'left' } });
      let $nextWrapper = $('<div/>', { class: 'br-next-wrapper', data: { pos: 'right' } });
      ...
      // init thumbs
      const $prevThumb = $('<div/>', { class: 'br-nav-thumb' }).on('click' + this._namespace, $.proxy(this.prevSlide, this)).prependTo($prevWrapper);
      const $nextThumb = $('<div/>', { class: 'br-nav-thumb' }).on('click' + this._namespace, $.proxy(this.nextSlide, this)).appendTo($nextWrapper);
      let content = '';
      ...
      $wrappers.each($.proxy(function (n, el) {
        ...
        const $imgs = $thumb.children().addTransitionClass('br-opacity-transition');
      ...
      if (this._options.navButtons === 'large') {
        $wrappers.each($.proxy(function (n, el) {
          ...
          $(el).css(pos, -$thumb.outerWidth(true))
            .on('mouseenter' + this._namespace, { pos }, $.proxy(this.showNavWrapper, this))
            .on('mouseleave' + this._namespace, { pos }, this.hideNavWrapper);
        }, this));
      } else {
      ...
      this._$rotator.on('updateNavThumbs' + this._namespace, $.proxy(function () {
        ...
        const $nextItem = this._$currItem.brNext();
        const $prevImg = $prevThumb.children().eq($prevItem.index());
        const $nextImg = $nextThumb.children().eq($nextItem.index());
  ...
  showNavWrapper: function (e) {
    ...
    $wrapper.animate(prop, {
      duration: Rotator.ANIMATE_SPEED,
      queue: false,
      complete: function () {
  ...
  hideNavWrapper: function (e) {
    const $wrapper = $(e.currentTarget);
    const $thumb = $wrapper.find('>.br-nav-thumb');
    const margin = $thumb.data('margin') || 0;
    const prop = {};
  ...
  showNavThumb: function (e) {
    ...
    const $wrapper = $nav.data('wrapper').removeClass('br-shrink');
  ...
  hideNavThumb: function (e) {
    ...
    const $wrapper = $nav.data('wrapper');
      ...
      for (let i = 0; i < this._numItems; i++) {
        ...
        switch (this._options.thumbnails) {
          ...
          case 'text':
            ...
            break;
          case 'image': {
            const $img = (SUPPORT.backgroundSize ? $('<div/>') : $('<img/>', { alt: '' }));
  ...
  initLayer: function ($item) {
    ...
    $layers.each($.proxy(function (n, el) {
      ...
      $el.data({ opacity: getFloat($el.css('opacity'), 1), metric });
  ...
  displayTooltip: function ($thumb) {
    const $base = (this._options.cpanelOutside ? this._$extPanel : $thumb);
    let top; let left;

    if (this._orientation === 'vertical') {
      left = $base.offset().left;
      if (this._cpPosition.x === 'left') {
        left += $base.width();
      } else {
    ...
    this._tooltipId = setTimeout($.proxy(function () {
      this._$tooltip.css({ opacity: 0, top, left }).show().animate({ opacity: 1 }, Rotator.ANIMATE_SPEED,
        function () {
  ...
  displayLayers: function () {
    ...
    $wrapper.children().each($.proxy(function (n, el) {
      const $layer = $(el);
      const delay = getNonNegInt($layer.data('delay'), 0);
  ...
  animateLayer: function ($layer, out) {
    ...
    const dir = (out ? 'Out' : '');
    let effect = data['effect' + dir];
    const opts = { duration: parseInt(data['duration' + dir], 10), easing: data['easing' + dir], complete: $.noop };
  ...
  getMoveProps: function ($el, direction) {
    ...
    const dim = isHorizontal ? 'outerWidth' : 'outerHeight';
    let fwd = (direction === 'right' || direction === 'down');
    const from = {};
    const to = {};
    let pos; let translate; let side;
  ...
  getShiftProps: function ($el, direction) {
    const isHorizontal = (direction === 'left' || direction === 'right');
    let inverse = (direction === 'right' || direction === 'down' ? -1 : 1);
  ...
  displayContent: function ($item) {
    ...
    if (effectOff || ($.inArray(data.effect, Effect.EFFECTS) < 0 && data.effect !== 'random')) {
  ...
  loadThumb: function ($thumb, fadeIn) {
    const deferred = $.Deferred();
    const $img = ($thumb.is('img') ? $thumb : $('<img/>'));
  ...
  processImage: function ($item) {
    ...
    const $img = $item.find('>img.br-img');
    const position = $item.data('imagePosition');
    const arr = (position + '').split(' ', 2);
    ...
    if (this._options.responsive) {
      ...
      const props = { top: parseInt($img.css('top'), 10), left: parseInt($img.css('left'), 10), width: $img.width(), height: $img.height() };
  ...
  addThumbScroll: function () {
    ...
    if (!IS_TOUCH) {
      ...
      const pos = data.pos;
      const dim = data.dim;
      const pagePos = 'page' + capitalize(data.coord);
      ...
      this._$thumbPanel.on('mouseenter' + this._namespace, function () {
        ...
        .on('mousemove' + this._namespace, $.proxy(function (e) {
          ...
          this._$thumbList.animate(prop, { duration: Rotator.ANIMATE_SPEED, easing, queue: false });
        }, this));
    }
  },
  ...
  syncThumbs: function () {
    if (!this._$thumbPanel.hasClass('br-hover-on')) {
      const data = this._$cpanel.data();
      const thumb = this._$thumbs.eq(this._activeIndex).offset()[data.pos];
      const panel = this._$thumbPanel.offset()[data.pos];
      const prop = {};
  ...
  touchMove: function (e) {
    ...
    const yDist = this._startY - e.originalEvent.touches[0].pageY;
